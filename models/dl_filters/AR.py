...
class ARTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=4, num_layers=4, dim_feedforward=512, dropout=0.):
        ...
        self.transformer = nn.TransformerEncoder(layers, num_layers)
    ...
    def forward(self, x, target=None):
        ...
        mask = self._get_mask(T + 1, x.device)
        output = self.transformer(x, mask)  # [B, T+1, D]

        x_ar = output[:, 1:, :]  # [B, T, D]

        if target is not None:
            ar_loss = F.mse_loss(x_ar, target)
            return x_ar, ar_loss
        else:
            return x_ar
...
class AR(nn.Module):
    def __init__(self, kernel_size=5, bias=False, _lambda=2.):
    ...
    def forward(self, x_n):
        ...
        f_ar, ar_loss = self.transformer(f_c, f_c, training=True)
        rec_c = self.decoder(f_ar.permute(0, 2, 1))
        
        t_loss_c = F.mse_loss(rec_c, x_c)
        
        tc_loss = self._lambda * self.base_weight * ar_loss + \
                  self.base_weight * t_loss_c
    ...
    def inference(self, x):
        ...
        x = self.encoder(x)
        x = x.permute(0, 2, 1)
        rec = self.decoder(self.transformer(x).permute(0, 2, 1))
        return rec