...
def train_diffusion(model, config, dataset, device, foldername="", log_dir=None):
    ...
    for epoch_no in range(config["epochs"]):
        ...
        for batch_no, (y_gt, input) in enumerate(train_loader, start=1):
            y_gt, input = y_gt.to(device), input.to(device)
            optimizer.zero_grad()
            
            loss = model(y_gt, input)
            loss.backward()
        ...
        if (epoch_no + 1) % config['val_interval'] == 0:
            ...
            with torch.no_grad():
                with tqdm(val_loader) as it:
                    for batch_no, (y_gt, input) in enumerate(it, start=1):
                        y_gt, input = y_gt.to(device), input.to(device)
                        
                        if config['use_ema']:
                            output = ema.ema_model.denoising(input)
                        else:
                            output = model.denoising(input)
                        loss = SSDLoss()(output, y_gt).item()
                        avg_val_loss += loss
    ...
    for epoch_no in range(config["epochs"]):
        ...
        with tqdm(train_loader) as it:
            for batch_no, (y_gt, input) in enumerate(it, start=1):
                ...
                denoised_ecg = generator(input, z)
                
                optimizer_D.zero_grad()
                
                real_pair = torch.cat([y_gt, input], dim=1)
                real_pred = discriminator(real_pair)
                real_loss = lsgan_loss(real_pred, torch.ones_like(real_pred))
                
                fake_pair = torch.cat([denoised_ecg.detach(), input], dim=1)
                ...
                fake_pair = torch.cat([denoised_ecg, input], dim=1)
                fake_pred = discriminator(fake_pair)
                g_adv_loss = lsgan_loss(fake_pred, torch.ones_like(fake_pred))
                
                g_l1_loss = torch.nn.L1Loss()(denoised_ecg, y_gt)
            ...
            with torch.no_grad():
                with tqdm(val_loader) as it:
                    for batch_no, (y_gt, input) in enumerate(it, start=1):
                        ...
                        Val_loss = torch.nn.L1Loss()(denoised_ecg, y_gt)
...
def train_dl(model, config, dataset, device, foldername="", log_dir=None):
    ...
    for epoch_no in range(config["epochs"]):
        ...
        for batch_no, (y_gt, input) in enumerate(train_loader, start=1):
            ...
            ssd_value = SSDLoss()(output, y_gt).item()
    ...
    with tqdm(initial=step, total=config['steps']) as pbar:
        while step < config['steps']:
            ...
            for _ in range(config['gradient_accumulate_every']):
                try:
                    if config['condition']:
                        y_gt, input = next(data_iter)
                        y_gt, input = y_gt.to(device), input.to(device)
                        data = [y_gt, input]
                    else:
                        y_gt = next(data_iter)
                        y_gt = y_gt[0] if isinstance(y_gt, list) else y_gt
                        y_gt = y_gt.to(device)
                        data = y_gt
                    ...
                    with torch.no_grad():
                        with tqdm(test_loader) as it:
                            for batch_no, (y_gt, input) in enumerate(it, start=1):
                                ...
                                if config['use_ema']:
                                    [_, output] = ema.ema_model.sample([input, 0], batch_size=input.shape[0])
                                else:
                                    [_, output] = model.sample([input, 0], batch_size=input.shape[0])
                                loss = SSDLoss()(output, y_gt).item()
                                avg_val_loss += loss
...
def train_flow(model, config, dataset, device, foldername="", log_dir=None):
    ...
    with tqdm(initial=step, total=config['steps']) as pbar:
        while step < config['steps']:
            ...
            for _ in range(config['gradient_accumulate_every']):
                try:
                    y_gt, input, snr = next(data_iter)
                    y_gt, input, snr = y_gt.to(device), input.to(device), snr.to(device)
                except StopIteration:
                    data_iter = iter(train_loader)
                    y_gt, input, snr = next(data_iter)
                    y_gt, input, snr = y_gt.to(device), input.to(device), snr.to(device)

                loss, _ = model(input, y_gt, self_cond=input, snr=snr, step=step)
                    ...
                    with torch.no_grad():
                        with tqdm(val_loader) as it:
                            for batch_no, (y_gt, input) in enumerate(it, start=1):
                                ...
                                if config['use_ema']:
                                    [output, _] = ema.ema_model.sample(input, self_cond=input)
                                else:
                                    [output, _] = model.sample(input, self_cond=input)
...
def train_ar(model, config, dataset, device, foldername="", log_dir=None):
    ...
    for epoch_no in range(config["epochs"]):
        ...
        with tqdm(train_loader) as it:
            for batch_no, (_, input) in enumerate(it, start=1):
                input = input.to(device)
                optimizer.zero_grad()
                
                loss, loss_c, loss_ar = model(input)
                loss.backward()
                optimizer.step()
