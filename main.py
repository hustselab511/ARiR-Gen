...
if __name__ == "__main__": 
    ...
    parser.add_argument('--use_rmn', action='store_true', default=False, help='Add Random Mixed Noise')
    ...
    if args.train_set == 'QT':
        dataset = ECGDataset(n_type=args.n_type, use_rmn=args.use_rmn, config=config['dataset'])
    elif args.train_set == 'SimEMG':
        dataset = EMGDataset(n_type=args.n_type, config=config['dataset'], train=True)
    ...
    if (args.exp_name == "ARiRGen"):
        from models.generation_filters.Backbone import Unet
        from models.generation_filters.ARiRGen import CFM
    ...
    # AR 
        ...
        model = AR(**config['model']).to(args.device)
